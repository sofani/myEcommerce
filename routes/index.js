...
router.get('/', function(req, res, next) {
  ...
  res.render('shops/index', { title: 'Eritrean Resturant', products: productChunks, successMsg: successMsg, noMessage: !successMsg});
...
router.get('/checkout', isLogedIn,  function(req, res, next) {
      if (!req.session.cart) {
        return res.render('shops/shopping-cart', {products: null});
      }
      var cart = new Cart(req.session.cart);
...
router.post('/checkout', isLogedIn, function(req, res, next) {
     if (!req.session.cart) {
        return res.render('shops/shopping-cart', {products: null});
      }
      var cart = new Cart(req.session.cart);
      var stripe = require("stripe")(
           "sk_test_Zy9tDV9EfMWK6nTzWUgRyxIp"
      );
